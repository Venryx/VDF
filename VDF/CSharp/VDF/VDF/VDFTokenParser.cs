...
public static class VDFTokenParser
{
	...
	class DepthData
	{
		public VDFToken startMarker;
		public VDFToken lastItemFirstToken;
		public bool needsChildStartBracket;
		public bool needsChildEndBracket;
	}
	static void PostProcessTokens(List<VDFToken> tokens)
	{
		...
		var depthData = new List<DepthData> {new DepthData()}; // add empty item, for level-0
		for (var i = 0; i < tokens.Count; i++)
		{
			var token = tokens[i];
		...
		var indentDepthsNeedingEndBracket = new List<int>();
		for (var i = 0; i < tokens.Count; i++)
		{
			var token = tokens[i];
