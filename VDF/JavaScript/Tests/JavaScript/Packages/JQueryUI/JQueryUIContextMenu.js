...
// Note: we don't want to unbind the 'contextmenu' event
		...
		{
			// custom changed
			var $entry = this.$menu.find("li a[href=" + (normCommand(cmd).length ? "#" + normCommand(cmd) : "") + "]");
...
// hyphen, em dash, en dash: separator as defined by UI Menu 1.10
			...
			} else
			{
				...
				if (normCommand(entry.cmd).length)
					$a.attr("href", "#" + normCommand(entry.cmd));
		...
		createMenuMarkup: function (options, $parentUl)
		{
			...
			if ($parentUl == null)
			{
				...
				$parentUl.addClass("contextMenu"); // custom
			}
