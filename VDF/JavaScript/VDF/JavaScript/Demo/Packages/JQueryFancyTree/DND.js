...
function _initDragAndDrop(tree) {
	...
	if(dnd && dnd.dragStart )
	{
		tree.widget.element.draggable(
		{
			addClasses: false,
			appendTo: "body",
			containment: tree.widget.element, // custom changed
			delay: 0,
			distance: 4,
			...
			{
				window.fancyTreeStartScrollTop = tree.widget.element.children().scrollTop(); // custom
			...
			{
				var scrollFrame = tree.widget.element.children();
				var dragHelper = $("div.fancytree-drag-helper");
				var scrollTopDif = scrollFrame.scrollTop() - window.fancyTreeStartScrollTop;

				var top = parseInt(dragHelper.css("top"));
				var targetTop = top + scrollTopDif;
				dragHelper.css("marginTop", targetTop - top);
			}
		});
	}
...
//      helper: null,
		...
		allowResorting: true, // custom
		allowDropIntoNonFolder: true // custom
	},
...
//                break;
				...
				// custom changed (disabled misleading drop-after-expanded-node action; maybe temp)
				var isExpandedNode = $(node.li).children("ul:visible").length;
				if( enterResponse.after && relPos2.y > 0.75 && !isExpandedNode)
					hitMode = "after";
				else if(!enterResponse.over && enterResponse.after && relPos2.y > 0.5 && !isExpandedNode)
					hitMode = "after";
				...
				if(dnd.preventVoidMoves)
				{
					...
					// custom
					if (!this.options.dnd.allowResorting)
					{
						if(hitMode === "over" && otherNode && otherNode.parent === node) // if dropping node onto its own parent
						{
							logMsg("    Child-drop over own parent prevented.");
							hitMode = null;
						}
						else if ((hitMode === "before" || hitMode === "after") && otherNode.parent == node.parent) // if dropping node before/after one of its own siblings
						{
							logMsg("    Child-drop before/after own sibling prevented.");
							hitMode = null;
						}
					}
					if (!this.options.dnd.allowDropIntoNonFolder && hitMode === "over" && !node.isFolder())
					{
						logMsg("    Child-drop into non-folder prevented.");
						hitMode = null;
					}
				}
