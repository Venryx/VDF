...
namespace Utilities
{
    ...
    class GlobalKeyboardHook
    {
        ...
        KeyboardHookProc hookProcInstance; // for fixing delegate garbage-collection issue
        ...
        public GlobalKeyboardHook() { Hook(); }

        /// <summary>Releases unmanaged resources and performs other cleanup operations before the <see cref="GlobalKeyboardHook"/> is reclaimed by garbage collection and uninstalls the keyboard hook.</summary>
        ...
        public void Hook()
        {
            ...
            hookProcInstance = HookProc; // for fixing delegate garbage-collection issue
            hhook = SetWindowsHookEx(WH_KEYBOARD_LL, hookProcInstance, hInstance, 0); // second parameter needs to use the class reference, to ensure it isn't garbage collected
        }

        /// <summary>Uninstalls the global hook.</summary>
        public void Unhook() { UnhookWindowsHookEx(hhook); }
        ...
        int HookProc(int code, int wParam, ref KeyboardHookStruct lParam)
        {
            if (code >= 0)
            {
                ...
                //if (hookedKeys.Contains(key))
                {
                    ...
                    {
                        keysDown.Add(kea.KeyCode);
                        if(KeyDown != null)
                            KeyDown(this, kea);
                    }
                    else if (wParam == WM_KEYUP || wParam == WM_SYSKEYUP)
                    {
                        keysDown.Remove(kea.KeyCode);
                        if (KeyUp != null)
                            KeyUp(this, kea);
                    }
                    if (kea.Handled)
        ...
        [DllImport("user32.dll")] static extern IntPtr SetWindowsHookEx(int idHook, KeyboardHookProc callback, IntPtr hInstance, uint threadId);
        ...
        /// <returns></returns>
        [DllImport("user32.dll")] static extern int CallNextHookEx(IntPtr idHook, int nCode, int wParam, ref KeyboardHookStruct lParam);
